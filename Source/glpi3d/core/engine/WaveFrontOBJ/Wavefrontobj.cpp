...
bool OBJMesh::CalculateNormal(bool flatShading)
{
    ...
    if( objMeshModel.normals.size() == 0 ){
        ...
        glm::vec3 a, b, c;
        for(unsigned int i=0; i<objMeshModel.indices.size();i += 3)
        {
        ...
        if ( !flatShading ){
            for(unsigned int i = 0; i < objMeshModel.vecFaceIndex.size(); i++)
            {
        ...
        for (unsigned int j=0;j<objMeshModel.vertices.size(); j++){
...
{
    int faceIdxSize = objMeshModel.vecFaceIndex.size();
...
bool OBJMesh::ScanFaceIndex( FILE* pFile, char c )
{
    ...
    if ( objMeshModel.normals.size() && objMeshModel.uvs.size() )
    {
        ...
        while(fgetc(pFile) != '\n'){
            ...
            scanCount = (float)fscanf(pFile,"%d/%d/%d",&vIndex,&uvIndex,&nIndex);
    ...
    else if( objMeshModel.normals.size() && !objMeshModel.uvs.size() )
    {
        while(fgetc(pFile) != '\n'){
            vIndex = nIndex = uvIndex = 0;
            scanCount = (float)fscanf(pFile,"%d//%d",&vIndex, &nIndex);
    ...
    else if( !objMeshModel.normals.size() && objMeshModel.uvs.size() )
    {
        while(fgetc(pFile) != '\n'){
            vIndex = nIndex = uvIndex = 0;
            scanCount = (float)fscanf(pFile,"%d/%d/",&vIndex, &uvIndex);
    ...
    else if( !objMeshModel.normals.size() && !objMeshModel.uvs.size() )
    {
        vIndex = nIndex = uvIndex = 0;
        while(fgetc(pFile) != '\n'){
            scanCount = (float)fscanf(pFile,"%d",&vIndex);
