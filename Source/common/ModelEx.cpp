...
Model::Model(Scene*	handler, Model* model, ModelType type, std::string objectName):ObjectRelative(objectName, model)
{
...
void Model::Initialize()
{
    for(int i =0; i<childList.size(); i++){
        dynamic_cast<Model*>(childList.at(i))->Initialize();
    }
}
...
{
    transformationLocal = glm::translate( transformationLocal, center);
...
void Model::Render(bool (*customRender)())
{
    for(int i =0; i<childList.size(); i++){
