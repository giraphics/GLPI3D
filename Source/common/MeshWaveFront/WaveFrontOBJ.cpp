...
bool OBJMesh::CreateInterleavedArray()
{
	...
	objMeshModel.indexCount = (int)objMeshModel.indices.size();
...
bool OBJMesh::ScanMaterial(FILE* pFile, char c)
{
	char keyword[256];
	char file[256];
	...
	int scanCount;
	//while (fgetc(pFile) != '\n'){
	scanCount = fscanf(pFile, "%s %s", &keyword, &file);
	//}

	return true;
}

bool OBJMesh::ScanMaterialFile(FILE* pFile, const char* meshFilePath)
{
	char meshFilePathTemp[256];
	char keyword[256];
	char alterPath[256]; //Assuming the material file into the same folder.
	char path[256];
	bool   eofReached = false;
	char   c;
	int scanCount;
	//while (fgetc(pFile) != '\n'){
	scanCount = fscanf(pFile, "%s %s", &keyword, &path);
	//}path = 0x002c8f38 "../Resource/Models/mbclass.obj"
	//meshFilePath
	strcpy(meshFilePathTemp, meshFilePath);
	int len = strlen(meshFilePathTemp);
	for (int i = len - 1; i >= 0; i--){
		if (meshFilePathTemp[i] == '/'){
			meshFilePathTemp[i+1] = '\0';
			break;
		}
	}
	// 0 strncpy(newPath);
	strcat(meshFilePathTemp, path);
	FILE*  pFileMaterial;
	pFileMaterial = fopen(meshFilePathTemp, "r");
	if (pFileMaterial == NULL)
	{
		LOGI("\nFail to open Obj mesh file: %s", meshFilePathTemp);
		fclose(pFileMaterial);
		return false;
	}
	while (!eofReached)
	{
		c = fgetc(pFileMaterial);
		switch (c)
		{
		case '#': // Comment. Ignore
			while (fgetc(pFileMaterial) != '\n'); // Skip till new next line not reached.
			break;
		...
		//	c = fgetc(pFileMaterial); // The next character will let us know what vertex attribute to load
		//	ScanVertexNormalAndUV(pFileMaterial, c);
		//	break;

		//case 'f':// 'f' means it is a face index information in the form of v/u/n
		//	ScanFaceIndex(pFileMaterial, c);
		//	break;

		case 'n': //newmtl materialName
			char materialName[256];
			scanCount = fscanf(pFileMaterial, "%s %s", &keyword, &materialName);
			if (strcmp("ewmtl", keyword) == 0){
				//objMeshModel
			}
		//	//ScanMaterial(pFileMaterial, path);
			break;
...
bool OBJMesh::ParseFileInfo(const char* path)
{
    FILE*  pFile;
    bool   eofReached = false;
    char   c;
...
//#endif
			...
			case 'm':// 'f' means it is a face index information in the form of v/u/n
				//ScanMaterialFile(pFile, path);
				break;
...
Mesh* OBJMesh::ParseObjModel(const char* path, bool flatShading)
{
